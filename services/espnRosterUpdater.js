...
export class ESPNRosterUpdater {
  ...
  /**
   * The NFL team's abbreviation, from the one map that has it.
   *
   * This used to carry its own copy of all 32 — a second definition of a fact
   * that `getNFLTeamAbbreviation` already owns, and one more place to forget
   * when a franchise relocates. The empty-string fallback is kept rather than
   * that function's `null`, because this value is written to
   * `players.pro_team_name` and the callers here have always sent a string.
   */
  getProTeamName(proTeamId) {
    return getNFLTeamAbbreviation(proTeamId) || '';
  }
