...
# Scrapes weekly projections from multiple sources using ffanalytics
...
# Function to scrape weekly data with enhanced error handling
scrape_weekly_data <- function(week = NULL, season = NULL, sources = NULL, positions = NULL) {
  log_message("INFO", "Starting weekly data scraping")
  ...
  log_message("INFO", sprintf("Scraping weekly data for week %d, season %d", week, season))
  log_message("INFO", sprintf("Sources: %s", paste(sources, collapse = ", ")))
  log_message("INFO", sprintf("Positions: %s", paste(positions, collapse = ", ")))
  
  # Validate inputs
  if(week < 1 || week > 18) {
    error_msg <- sprintf("Invalid week number: %d. Must be between 1 and 18", week)
  ...
  tryCatch({
    log_message("INFO", "Initiating data scraping from sources")
    
    # Scrape data with timeout handling
    scraped_data_raw <- tryCatch({
      scrape_data(src = sources, pos = positions, season = season, week = week)
    }, error = function(e) {
      log_message("ERROR", sprintf("Failed to scrape data: %s", e$message))
      return(NULL)
    })
    
    if(is.null(scraped_data_raw) || length(scraped_data_raw) == 0) {
      error_msg <- "No data scraped from any sources"
      log_message("ERROR", error_msg)
      return(list(success = FALSE, error = error_msg, data = NULL))
    }
    ...
    if(is.null(scraped_data) || nrow(scraped_data) == 0) {
      error_msg <- "No data scraped from any sources after combining"
      log_message("ERROR", error_msg)
      return(list(success = FALSE, error = error_msg, data = NULL))
    }
    
    log_message("INFO", sprintf("Successfully scraped %d raw records", nrow(scraped_data)))
    
    # Create projections manually (bypassing projections_table issues)
    log_message("INFO", "Creating projections from scraped data")
    projections <- tryCatch({
      ...
      log_message("ERROR", sprintf("Failed to create projections: %s", e$message))
      return(NULL)
    })
    
    if(is.null(projections) || nrow(projections) == 0) {
      error_msg <- "Failed to generate projections from scraped data"
      log_message("ERROR", error_msg)
      return(list(success = FALSE, error = error_msg, data = NULL))
    }
    
    # Add basic analytics data (simplified approach)
    log_message("INFO", "Adding basic analytics data")
    
    projections <- tryCatch({
      projections %>%
        dplyr::mutate(
          # Add basic ranking based on points
          weekly_rank = row_number(desc(points)),
          ...
          # Add basic consistency rating (placeholder)
          consistency_rating = 0.75,
          # Use points as ceiling/floor estimates
          ceiling_score = points * 1.2,
          floor_score = points * 0.8,
          # Add metadata
          week = week,
          season = season,
          scraped_at = Sys.time()
      ...
      log_message("WARN", sprintf("Failed to add analytics data: %s", e$message))
      projections
    })
    
    log_message("INFO", sprintf("Successfully processed %d player projections", nrow(projections)))
    
    # Validate final data
    if(is.null(projections) || nrow(projections) == 0) {
      error_msg <- "No valid projections generated"
      log_message("ERROR", error_msg)
      return(list(success = FALSE, error = error_msg, data = NULL))
    }
    ...
    log_message("INFO", "Weekly data scraping completed successfully")
    ...
    error_msg <- sprintf("Unexpected error during scraping: %s", e$message)
    log_message("ERROR", error_msg)
    return(list(success = FALSE, error = error_msg, data = NULL))
  })
}
...
if (!interactive()) {
  ...
  # Execute scraping
  result <- scrape_weekly_data(week, season, sources, positions)
