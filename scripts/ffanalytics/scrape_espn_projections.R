...
# Scrapes projections from ESPN only using ffanalytics
library(ffanalytics)
library(jsonlite)
library(dplyr)
...
# Function to scrape ESPN data
scrape_espn_data <- function(week = NULL, season = NULL, positions = NULL) {
  ...
  cat("Scraping ESPN data for week", week, "season", season, "\n")
  cat("Positions:", paste(positions, collapse = ", "), "\n")
  
  tryCatch({
    # Scrape data from ESPN only
    scraped_data <- scrape_data(src = "ESPN", pos = positions, season = season, week = week)
    
    if(is.null(scraped_data) || nrow(scraped_data) == 0) {
      cat("No ESPN data scraped\n")
      return(list(success = FALSE, error = "No ESPN data scraped", data = NULL, source = "ESPN"))
    }
    ...
    projections$source <- "ESPN"
    
    cat("Successfully scraped", nrow(projections), "ESPN player projections\n")
    ...
    cat("Error scraping ESPN data:", e$message, "\n")
    return(list(success = FALSE, error = e$message, data = NULL, source = "ESPN"))
  })
}
...
if (!interactive()) {
  ...
  # Execute scraping
  result <- scrape_espn_data(week, season, positions)
