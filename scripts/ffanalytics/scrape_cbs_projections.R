...
# Scrapes projections from CBS only using ffanalytics
library(ffanalytics)
library(jsonlite)
library(dplyr)
...
# Function to scrape CBS data
scrape_cbs_data <- function(week = NULL, season = NULL, positions = NULL) {
  ...
  cat("Scraping CBS data for week", week, "season", season, "\n")
  cat("Positions:", paste(positions, collapse = ", "), "\n")
  
  tryCatch({
    # Scrape data from CBS only
    scraped_data <- scrape_data(src = "CBS", pos = positions, season = season, week = week)
    
    if(is.null(scraped_data) || nrow(scraped_data) == 0) {
      cat("No CBS data scraped\n")
      return(list(success = FALSE, error = "No CBS data scraped", data = NULL, source = "CBS"))
    }
    ...
    cat("Successfully scraped", nrow(projections), "CBS player projections\n")
    ...
    cat("Error scraping CBS data:", e$message, "\n")
    return(list(success = FALSE, error = e$message, data = NULL, source = "CBS"))
  })
}
...
if (!interactive()) {
  ...
  # Execute scraping
  result <- scrape_cbs_data(week, season, positions)
